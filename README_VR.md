...
- `--vr-walk-edge-stop=<meters>` – stop before ledges by this distance (default `0.25`)
- `--vr-walk-ground-probe=<meters>` – downward probe distance for ground snap (default `1.0`)
- `--vr-walk-replan=<on|off>` – enable local replanning around obstacles (default `on`)
...
- `--vr-ui-scroll-accel=<float>` – scroll wheel acceleration for HUD laser (default `1.0`)
